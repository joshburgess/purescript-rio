-- | Integration tests for `RIO.Aff.Postgres.Notify` against a real
...
module Test.RIO.Aff.Postgres.NotifySpec (spec) where
...
import RIO.Aff.Error (catchTag, fail) as RIO
import RIO.Aff.Postgres (PgError, Postgres, pgErrorMessage, withTransaction)
import RIO.Aff.Postgres.Layer (postgresLayer)
import RIO.Aff.Postgres.Notify (Notify, notify, notifyUsing, withListen)
...
spec conn = do
  describe "RIO.Aff.Postgres.Notify (integration)" do
