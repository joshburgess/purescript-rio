...
-- |
-- | Wraps any `RIO` action so it runs inside a `RIO.Logger`
...
-- | unchanged.
module RIO.HTTPurple.Middleware
  ( withRequestContext
  ) where
...
import RIO.Local (Local, locally)
import RIO.Logger (Logger, logError, logInfo, withFields)
