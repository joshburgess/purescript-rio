-- | File-backed `Source`s for `RIO.Config`.
-- |
...
-- |   shape matches the way `RIO.Config.nested` qualifies keys, so
...
-- |
-- | ```purescript
-- | import RIO.Config (load)
-- | import RIO.Config.File (jsonFileSource)
-- |
-- | main = launchAff_ do
-- |   src <- jsonFileSource "config.json"
-- |   runRIO' (load (Proxy :: _ "config") src appConfig program)
-- | ```
module RIO.Config.File
  ( dotenvFileSource
  , jsonFileSource
  , parseDotenv
  , flattenJson
...
import RIO.Config (Source, mkSource)
