-- | Integration tests for `RIO.Postgres.Json`. Exercises the
-- | round-trip of typed values through a real `jsonb` column.
module Test.RIO.Postgres.JsonSpec (spec) where
...
import RIO.Postgres.Json (JsonB(..))
...
spec conn = do
  describe "RIO.Postgres.Json (integration)" do
