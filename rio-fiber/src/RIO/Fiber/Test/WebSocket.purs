...
-- |   rec <- liftEffect (newRecordingWebSocket [ TextMessage "hello" ])
-- |   conn <- fromAff (rec.webSocket.connect "ws://test")
-- |   m <- fromAff conn.receive
-- |   case m of
-- |     Just msg -> fromAff (conn.send msg)
-- |     Nothing -> pure unit
-- |   snap <- liftEffect rec.snapshot
-- |   liftEffect (snap.connects `shouldEqual` [ "ws://test" ])
-- |   liftEffect (snap.sent `shouldEqual` [ Tuple "ws://test" (TextMessage "hello") ])
-- | ```
module RIO.Fiber.Test.WebSocket
  ( RecordingWebSocket
  , RecordingSnapshot
...
import RIO.Fiber.WebSocket (Message, WebSocket, WebSocketConnection, mockWebSocket)
...
newRecordingWebSocket
  :: Array Message
  -> Effect RecordingWebSocket
newRecordingWebSocket initialInbox = do
