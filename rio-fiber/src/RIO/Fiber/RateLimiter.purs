...
-- | token count and the timestamp of its last refill; reads of the
-- | clock happen through `RIO.Fiber.Clock` so the limiter is fully
-- | driven by a swapped-in fake clock in tests.
-- |
-- | `acquire` (and `acquireN`) deduct tokens from the bucket if
-- | enough are available, otherwise compute the wait until enough
-- | have refilled and `sleep` for that long before retrying.
-- | `tryAcquire` returns immediately with a `Boolean` indicating
-- | whether the deduction succeeded. `withPermit` brackets an action
-- | with an acquire (and registers no release because tokens are
-- | spent, not borrowed).
-- |
-- | ## Fairness
-- |
-- | The implementation does not maintain a queue of waiters: each
-- | acquirer independently samples the bucket, sleeps, and retries.
-- | Under contention you should expect approximate fairness rather
-- | than FIFO ordering. If you need strict FIFO wakeups, layer a
-- | `Semaphore` (for the queue) on top of this (for the token math).
-- |
-- | ```purescript
-- | callApi :: forall r e. RateLimiter -> Request -> RIO r e Response
-- | callApi rl req = RateLimiter.withPermit rl (doCall req)
-- | ```
module RIO.Fiber.RateLimiter
  ( RateLimiter
  , Config
  , available
  , make
...
-- | A token-bucket rate limiter. Allocate with `make`; use through
-- | `acquire` / `tryAcquire` / `withPermit`.
...
-- | very first call to `acquire` (up to `burst`) succeeds without
-- | sleeping.
make :: forall r e. Config -> RIO r e RateLimiter
make cfg = do
...
acquire :: forall r e. RateLimiter -> RIO r e Unit
acquire = acquireN 1

-- | Acquire `n` permits, blocking until the bucket has accrued at
-- | least `n` tokens. If `n` exceeds the bucket's capacity, the
-- | call will never succeed; callers should keep `n` within the
-- | configured `burst`.
acquireN
  :: forall r e
   . Int
...
tryAcquire :: forall r e. RateLimiter -> RIO r e Boolean
...
-- | Run `action` after acquiring one permit. The permit is spent,
-- | not borrowed: there is no matching release.
withPermit
  :: forall r e a
   . RateLimiter
  -> RIO r e a
