...
-- | `Array` handles awkwardly: concatenation is O(n+m) for `Array`,
-- | which means folding a long stream of small arrays into a single
-- | `Array` costs O(n^2) in total. `Chunk` makes the concatenation
-- | O(1) by representing the catenated shape as a small tree, then
-- | defers the linearisation cost until the caller actually asks
-- | for it.
-- |
-- | ### Big-O summary
-- |
...
-- |   * `head`, `tail`: O(depth) average; `tail` rebuilds the rest
-- |     of the chunk
-- |   * `map`, `foldl`, `foldr`: O(n)
-- |   * `materialize`: O(n), one-shot flatten into a single
-- |     `Singleton` node so subsequent `toArray` / `index` calls are
-- |     cheap
-- |
...
-- |     `Array.length arr`. Most `Chunk`s end up in this shape after
-- |     `materialize`.
-- |   * `Concat Int (Chunk a) (Chunk a)`: the catenation of two
-- |     sub-chunks. The first field is the cached length so we don't
-- |     have to walk the tree twice.
-- |
...
-- |
-- | This is the moment to pay the linearisation cost; before it, the
-- | catenated shape stays as a tree of `Concat` nodes.
...
-- | tree-shaped chunk; `Concat` caches the result, so repeat reads
-- | are O(1).
...
-- | Every value except the first. The result is `empty` for an empty
-- | input.
