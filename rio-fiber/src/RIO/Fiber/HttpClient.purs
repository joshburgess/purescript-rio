...
-- |
-- | `RIO.Fiber.HttpClient` defines the *shape* of an HTTP transport so
...
-- | ```
-- |
-- | The service operation lives in `Aff` so backends can use the
-- | existing `purescript-aff` async machinery; `send` bridges to
-- | the fiber runtime via `RIO.Fiber.Aff.fromAff`, which propagates
-- | cancellation back into the underlying Aff fiber when the
-- | surrounding RIO fiber is interrupted.
-- |
...
-- | `(httpError :: HttpError | e)`. Combine with retry policies,
-- | with `RIO.Fiber.Tracer` for span instrumentation, or with
-- | `RIO.Fiber.Logger` for structured logs - the client
-- | deliberately does not bake any of those in, so the same call
-- | site works whether the program has those services or not.
-- |
-- | ```purescript
-- | example
-- |   :: forall r e
...
import RIO.Fiber.HttpStream (BodyStream)
import RIO.Fiber.Schema (DecodeError, Schema)
...
-- | and concurrency, and `fromAff` bridges its cancellation back
-- | into fiber interrupts at the call site.
type HttpClient =
  { sendRequest :: Request -> Aff (Either HttpError Response)
  }
...
-- | constructed by a dedicated module (e.g. `rio-fiber-node`).
...
send req = do
  client <- askAt (Proxy :: Proxy "httpClient")
  result <- fromAff (client.sendRequest req)
  case result of
    Left err -> fail (Variant.inj (Proxy :: Proxy "httpError") err)
...
decodeBody schema resp = case Schema.parseJson schema resp.body of
  Left e -> fail (Variant.inj (Proxy :: Proxy "httpError") (HttpDecode e))
  Right a -> pure a
...
ensureStatus resp =
  ...
  else fail (Variant.inj (Proxy :: Proxy "httpError") (HttpUnexpectedStatus resp))
