-- | Shape `RIO.Fiber.Test.Metrics` observations into the
-- | OTLP/JSON envelope used by OpenTelemetry collectors.
-- |
-- | The companion piece of `RIO.Fiber.Tracer.OTel`: the exporter
-- | is pure, takes an in-process list of recorded observations
-- | (the kind `RIO.Fiber.Test.Metrics.newRecordingMetrics`'s
-- | `snapshot` action returns), and produces the JSON document
-- | an OTLP receiver expects on its `/v1/metrics` HTTP endpoint.
-- | Performing the POST is the caller's job; pair the output
-- | with `RIO.Fiber.HttpClient.send` and a JSON body.
-- |
...
-- |   require bucket counts can derive them from the count/sum
-- |   or accept the simplified shape.
-- |
...
import RIO.Fiber.Test.Metrics (MetricKind(..), MetricRecord)
