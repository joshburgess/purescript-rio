...
-- |
-- | This is the same shape `RIO.Fiber.Schema.Branded` carries, but
-- | extracted so application code can adopt brands without
-- | depending on `RIO.Fiber.Schema`. The two are interoperable: a
...
-- | lookupUser :: UserId -> RIO r e (Maybe User)
...
-- | then call `mkBrand`. Pair `Brand` with `RIO.Fiber.Schema.refine`
