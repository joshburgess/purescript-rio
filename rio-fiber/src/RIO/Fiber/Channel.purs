...
-- | interop with `RIO.Fiber.Stream` and `RIO.Fiber.Sink`, and `pipe`
...
-- |         (fromSink Sink.collectAll))
-- | -- result :: Array Int = [1, 2, 3]
-- | ```
module RIO.Fiber.Channel
  ( Channel(..)
  , ChStep(..)
  , unChannel
  , done
...
import RIO.Fiber.Sink (Sink(..), SinkLoop) as Sink
import RIO.Fiber.Stream (Stream(..), Step(..)) as Stream
...
-- | Lift a `RIO.Fiber.Stream.Stream r e a` into a channel: a source
-- | that never reads, emits each `a`, and terminates with `unit`
-- | once the underlying stream is exhausted.
-- |
-- | The input and done parameters are polymorphic because a source
-- | channel does not constrain them.
fromStream :: forall r e i a. Stream.Stream r e a -> Channel r e i a Unit
fromStream (Stream.Stream pull) = Channel do
  step <- pull
  case step of
    Stream.Done -> pure (ChDone unit)
    Stream.Yield a rest -> pure (ChOut a (fromStream rest))

-- | Lift a `RIO.Fiber.Sink.Sink r e i a` into a channel: a sink
-- | that never emits, reads `i`s, and terminates with the sink's
-- | result of type `a`.
-- |
-- | The output parameter is polymorphic because a sink channel
-- | never emits. The sink's allocation step runs once at the moment
-- | the channel is first stepped, so per-run sink state (a `Ref`,
-- | typically) is scoped to this one run rather than shared across
-- | future `fromSink` calls on the same `Sink` value.
fromSink :: forall r e i o a. Sink.Sink r e i a -> Channel r e i o a
fromSink (Sink.Sink mk) = Channel do
  loop <- mk
  pure (loopToChStep loop)
  where
  loopToChStep :: Sink.SinkLoop r e i a -> ChStep r e i o a
  loopToChStep loop =
    ChIn
      ( \i -> Channel do
          mOut <- loop.step i
          case mOut of
            Just o -> pure (ChDone o)
            Nothing -> pure (loopToChStep loop)
      )
      (Channel (ChDone <$> loop.done))
...
-- |
-- | If the channel tries to read, the read returns the EOF branch
-- | immediately; if it tries to emit, the output is discarded. A
-- | truly closed pipeline (a source piped into a sink) reaches
-- | `ChDone` without invoking either.
