-- | Qualified-do sugar over `RIO.Fiber.Resource.acquireRelease`.
-- |
...
-- |
-- | ```purescript
-- | import RIO.Fiber.Resource.Do as Resource
-- |
...
-- | typed failure, defect, interrupt).
-- |
...
import RIO.Fiber.Resource (acquireRelease)
...
-- | `RIO.Fiber.Resource.acquireRelease`).
...
-- |
-- | The release receives the value produced by `acquire` and runs
-- | inside the uninterruptible release phase installed by
-- | `acquireRelease`: a late interrupt cannot abandon it midway.
-- | Release's error row is `()` because there is no caller-visible
-- | place to surface a typed failure from cleanup.
acquire
  :: forall r e a
   . RIO r e a
  -> (a -> RIO r () Unit)
