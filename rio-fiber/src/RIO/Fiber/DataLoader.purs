...
import RIO.Fiber.Core (RIO, failCause, liftEffect, sleep)
    ...
    case Map.lookup k entries of
      ...
      Nothing -> do
        ...
        when (not pending) do
          ...
          _ <- Scope.forkScoped rec.scope (flushLoop dl)
          pure unit
        pure d
  result <- Def.awaitPure slot
  case result of
    Right mv -> pure mv
    Left c -> failCause c
...
clear
  ...
  -> k
  -> RIO r e Unit
clear (DataLoader rec) k = Sem.withPermit rec.gate
...
clearAll
  :: forall r e k v
   . DataLoader r e k v
  -> RIO r e Unit
clearAll (DataLoader rec) = Sem.withPermit rec.gate
...
flushLoop dl@(DataLoader rec) = do
  sleep rec.window
  keys <- Sem.withPermit rec.gate do
...
runBatch (DataLoader rec) keys = do
  entries <- liftEffect (Ref.read rec.entries)
  cause <- F.causeOf (rec.batch keys)
  case cause of
    Right resultMap ->
      for_ keys \k -> case Map.lookup k entries of
        Just d -> do
          _ <- Def.succeed d (Right (Map.lookup k resultMap))
          pure unit
        Nothing -> pure unit
    Left c -> do
      -- Batch failed: re-raise the same cause for every waiter and
      -- evict their cache slots so a later load can retry.
      Sem.withPermit rec.gate do
        liftEffect (Ref.modify_ (\m -> foldlArray (\acc k -> Map.delete k acc) m keys) rec.entries)
      for_ keys \k -> case Map.lookup k entries of
        Just d -> do
          _ <- Def.succeed d (Left c)
          pure unit
        Nothing -> pure unit
