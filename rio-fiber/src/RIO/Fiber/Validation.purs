...
-- |
-- | Mirrors ZIO `Validation` and `Effect.Validation`. Compose with
-- | `<*>`, `<$>`, and `Apply.tuple`-style combinators to accumulate
-- | errors across independent fields; lift back into `RIO` with
-- | `toRIO` at the boundary where you want short-circuit behaviour
-- | to resume.
-- |
-- | ```purescript
...
-- | Project a `Validation` into the `Either` shape carrying every
-- | accumulated failure on the `Left`.
toEither
  :: forall e a
   . Validation e a
...
-- | flows through; on `Failure`, only the first accumulated failure
-- | is rethrown via the error row (the rest are dropped).
-- |
-- | Use this at the boundary where you want to "stop accumulating
-- | and resume short-circuiting": typically after validating an
-- | input record into a domain type and before running the rest of
-- | the program against that domain type.
toRIO :: forall r e a. Validation e a -> RIO r e a
toRIO = case _ of
  Success a -> pure a
  Failure es -> fail (NEArray.head es)
...
-- | returns the array of values. Otherwise, returns the concatenated
-- | list of every accumulated failure across the input.
-- |
-- | The order of accumulated failures matches the order of the input
-- | array.
collectAll
  :: forall e a
