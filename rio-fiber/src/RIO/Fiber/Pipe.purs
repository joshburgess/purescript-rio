...
-- | `o`. It is the natural middle ground between `Stream` (a producer)
-- | and `Sink` (a one-shot consumer): each input can emit zero or more
-- | outputs, and on end-of-stream the pipe gets a chance to emit any
-- | accumulated tail.
-- |
...
-- |
-- |   * `onInput :: i -> RIO r e { emit, more }` consumes one upstream
-- |     element. The emitted array is pushed downstream; `more = false`
-- |     signals "no more upstream — flush and stop".
...
-- | One step's emission. `more = false` means "I am done after these
-- | emissions — do not pull any more upstream".
type PipeStep o =
  { emit :: Array o
  , more :: Boolean
  }

-- | Build a pipe from a custom allocator. Most users want one of the
-- | prebuilts (`map`, `filter`, `chunked`, `take`, `mapAccum`).
...
-- | Emit `f a` for every input `a`. The pipe never terminates itself.
...
-- | Pass through the first `n` inputs and stop. Negative or zero `n`
-- | emits nothing.
take :: forall r e a. Int -> Pipe r e a a
take n
  | n <= 0 = Pipe (pure { onInput: \_ -> pure { emit: [], more: false }, onDone: pure [] })
  | otherwise = Pipe do
...
-- | Group inputs into fixed-size arrays. The trailing chunk emitted by
-- | `onDone` may be shorter than `n`. Non-positive `n` is treated as 1.
...
-- | Compose two pipes end-to-end: every output of the first pipe is
-- | fed as input to the second. If either pipe terminates with
...
andThen (Pipe mkP) (Pipe mkQ) = Pipe do
  p <- mkP
  q <- mkQ
  -- Once either side has gone "no more", we still need to fully flush
  -- the second pipe's trailing onDone before reporting it. A boolean
  -- ref tracks whether we've already finished.
