...
-- | set; on the next macrotask (`sleep (Milliseconds 0.0)`),
...
-- |
-- | ## Relationship to `RIO.Fiber.DataLoader`
-- |
-- | `RIO.Fiber.DataLoader` is the fiber-native equivalent built on
-- | `Scope`, `Semaphore`, and an `RIO`-typed batch function: failures
-- | propagate as full `Cause` trees and missing keys surface as
-- | `Nothing`. `Query` keeps the Aff-shaped `batchFn` and the
-- | `QueryError` row tag so code originally written against the aff
-- | port lifts unchanged. Pick whichever fits the call site.
module RIO.Fiber.Query
  ( Loader
  , Config
  , QueryError(..)
  , makeLoader
...
import Effect.Aff (Aff, attempt) as Aff
...
import RIO.Fiber.Concurrency (fork) as Concurrency
import RIO.Fiber.Core (RIO, liftEffect)
import RIO.Fiber.Deferred (Deferred)
import RIO.Fiber.Deferred as Def
import RIO.Fiber.Error (catchSome, mapError)
...
awaitOpen d = mapError Variant.expand (Def.await d)
...
flush (Loader ref) = do
  ...
  for_ chunks \batch -> do
    attempted <- fromAff (Aff.attempt (state.batchFn batch))
    case attempted of
      Left err -> do
        let msg = Exception.message err
        for_ batch \k -> case Map.lookup k pending of
          Nothing -> pure unit
          Just d -> void $ Def.fail d
          ...
          Just d -> case Map.lookup k resolved of
            Just v -> do
              when state.cacheEnabled $ liftEffect $ Ref.modify_
                ...
                ref
              void (Def.succeed d v)
            Nothing -> void $ Def.fail d
              ( Variant.inj (Proxy :: Proxy "queryError")
