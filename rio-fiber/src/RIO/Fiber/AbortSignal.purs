...
-- |
-- | Use this for HTTP, IndexedDB, and similar JS APIs that accept an
-- | abort signal in their options. The common case is `fetch`: a
-- | `RIO.Fiber.Core.async` caller has to allocate an
-- | `AbortController` by hand, thread the signal into the fetch
-- | options, and write the canceller as `() -> controller.abort()`.
-- | `asyncAbortable` removes that boilerplate.
module RIO.Fiber.AbortSignal
  ( AbortSignal
  , AbortController
  , newAbortController
  , signalOf
...
import RIO.Fiber.Core (RIO, async)
...
-- | observe `signal.aborted === true` and reject its in-flight work.
...
-- | An `async` variant that hands the register callback an
-- | `AbortSignal` plumbed to fiber interruption: when the fiber is
-- | interrupted, the signal aborts. The register receives the signal
-- | (for forwarding to `fetch` and friends) and the resume callback
...
asyncAbortable
  ...
  -> RIO r e a
asyncAbortable register = async \cb -> do
