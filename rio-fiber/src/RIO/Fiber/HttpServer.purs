...
-- |
-- | The application defines a `Handler :: ServerRequest -> Aff
-- | ServerResponse`. A driver (`rio-fiber-node`'s Node http
...
-- |
-- | Handlers run in `Aff`; lift `RIO`-shaped logic via
-- | `runAffEither` / `runAffThrow` from `RIO.Fiber.Aff` at
-- | handler-construction time (typically by closing over a
-- | concrete env record). The driver invokes the resulting `Aff`
-- | per request, so all RIO machinery (services, typed errors,
-- | bracketed resources) is available; the driver itself does
-- | not see the RIO row.
-- |
...
-- | streaming `StreamResponseBody` arm covers server-sent events
-- | and large file downloads.
module RIO.Fiber.HttpServer
  ( HttpServer
  , ServerRequest
  , ServerResponse
  , Handler
...
import RIO.Fiber.HttpClient (Method, RequestBody)
...
runHandler h req = fromAff (h req)
