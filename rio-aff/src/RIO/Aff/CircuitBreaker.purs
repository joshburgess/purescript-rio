...
-- | ```
-- |
-- | Like `RateLimiter`, the breaker reads time through `RIO.Aff.Clock`
-- | so tests can drive it deterministically with `RIO.Aff.Test.Clock`.
module RIO.Aff.CircuitBreaker
  ( CircuitBreaker
  , Config
  , Phase(..)
  , Snapshot
...
import RIO.Aff.Core (RIO, catchAll, fail, rethrow)
