...
-- |
-- | `RIO.Aff.OpenApi` is the data-only side of the API: a small set of
...
-- |
-- | The module reuses `RIO.Aff.Schema`'s `toJsonSchema` for body and
-- | parameter shapes, so a single `Schema a` value drives both
-- | wire decoding (`RIO.Aff.Schema.decode`) and the OpenAPI fragment.
-- |
...
import RIO.Aff.HttpClient (Method)
...
-- tags that `RIO.Aff.HttpClient.Method` carries.
