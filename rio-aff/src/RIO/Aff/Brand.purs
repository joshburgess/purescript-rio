...
-- |
-- | This is the same shape `RIO.Aff.Schema.Branded` carries, but
-- | extracted so application code can adopt brands without
-- | depending on `RIO.Aff.Schema`. The two are interoperable: a
...
-- | then call `mkBrand`. Pair `Brand` with `RIO.Aff.Schema.refine`
