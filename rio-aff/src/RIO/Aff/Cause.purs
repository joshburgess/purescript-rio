...
-- |
-- | `RIO.Aff.Error` already distinguishes typed failures (`fail`,
...
-- |
-- | Unlike `RIO.Aff.Concurrency.race` (which short-circuits on the
...
bothPar ra rb = mkRIO \r -> do
  ...
  pure case causeA, causeB, oa, ob of
    ...
    _, _, _, _ -> Left
      (Die (Exception.error "RIO.Aff.Cause.bothPar: impossible"))
...
-- | `RIO.Aff.Error.catchAll`: where `catchAll` only sees typed failures
...
-- | The cause-aware sibling of `RIO.Aff.Error.catchSome`: examine the
...
failCause cause = case leftmostLeaf cause of
  ...
  _ -> mkRIO \_ ->
    throwError (Exception.error "RIO.Aff.Cause.failCause: impossible")
...
-- | `RIO.Aff.Error.tapError`: where `tapError` only fires on typed
      ...
      case logged of
        ...
        Right _ -> case cause of
          ...
          _ -> case leftmostLeaf cause of
            ...
            _ -> throwError
              (Exception.error "RIO.Aff.Cause.tapErrorCause: impossible")

-- | Like `RIO.Aff.Concurrency.parTraverse`, but every branch runs to
...
-- | reach for `RIO.Aff.Concurrency.parTraverse` instead.
-- |
-- | ```purescript
...
-- | `RIO.Aff.Concurrency.race` (which surfaces the first completion,
...
-- | Cause-aware bracket. Like `RIO.Aff.Resource.acquireRelease`, but if
    ...
    Right (Right a) -> do
      ...
      pure case useOutcome, useCause, releaseCause of
        ...
        _, _, _ -> Left
          ( Die
              ( Exception.error
                  "RIO.Aff.Cause.acquireReleaseCause: impossible"
              )
          )
