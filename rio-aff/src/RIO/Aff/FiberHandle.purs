...
make scope = do
  ...
  addFinalizer scope do
    m <- liftEffect (Ref.read slot)
    case m of
      Just s -> case s.fiber of
        Fiber f ->
          Aff.killFiber
            (Aff.error "FiberHandle: scope closing")
            f.underlying
      Nothing -> pure unit
...
-- | returned fiber can be observed, joined, or interrupted manually;
-- | it also auto-clears from the slot on its own completion.
-- |
...
run (FiberHandle { slot, nextGen }) action = uninterruptible do
  ...
  case mPrev of
    Just s -> interrupt s.fiber
    Nothing -> pure unit
  fib <- fork action
  gen <- liftEffect do
    g <- Ref.modify (_ + 1) nextGen
    Ref.write (Just { fiber: fib, gen: g }) slot
    pure g
  -- Auto-clear the slot when this fiber finishes, but only if it is
  -- still the current occupant. A later `run` bumps `gen`; the
...
clear (FiberHandle { slot }) = do
  m <- liftEffect (Ref.read slot)
  case m of
    Just s -> do
      interrupt s.fiber
      liftEffect (Ref.write Nothing slot)
      pure true
    Nothing -> pure false
