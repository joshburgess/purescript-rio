...
-- | Effect-TS's `FiberRef` ship; `RIO.Aff.Local` is the simpler
...
import RIO.Aff.Concurrency (Fiber(..), mkFiber)
import RIO.Aff.Env (ask) as Env
import RIO.Aff.Internal (RIO(..), mkEffectRIO, mkRIO, unRIO, unsafeUnRIO)
...
-- | `RIO.Aff.Concurrency.forkScoped`.
forkFiberScoped
  :: forall r e e' a
   . Scope
  -> RIO (fiberRefs :: FiberRefs | r) e a
