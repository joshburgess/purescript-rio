...
-- |
-- | `RIO.Aff.HttpServer`'s `listen` / `shutdown` shape is about
...
-- |
-- | The recording shape mirrors `RIO.Aff.Test.Tracer.newRecordingTracer`:
-- | a record carrying both the service the program uses and the
-- | snapshot the test asserts against.
-- |
-- | ```purescript
-- | itRIO "responds 200 to /health" do
-- |   rec <- liftAff newRecordingHttpServer
-- |   resp <- liftAff (rec.dispatch handler healthRequest)
-- |   resp.status `shouldEqual` 200
-- |   calls <- liftEffect rec.snapshot
-- |   Array.length calls `shouldEqual` 1
-- | ```
module RIO.Aff.Test.HttpServer
  ( RecordingHttpServer
...
import RIO.Aff.HttpServer (Handler, HttpServer, ServerRequest, ServerResponse)
...
newRecordingHttpServer :: Aff RecordingHttpServer
newRecordingHttpServer = liftEffect do
