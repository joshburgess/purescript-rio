...
-- | as `Aff` defects, observable via `RIO.Aff.Error.sandbox`.
-- |
...
-- |
-- | Exposed because `RIO.Aff.FiberRef.forkFiber` needs to build a
...
-- | `RIO.Aff.Error.sandbox`. Joining a fiber that has already completed
...
-- |
-- | Resources held by the fiber via `RIO.Aff.Resource.acquireRelease`
...
-- | via `RIO.Aff.Error.sandbox`); they also interrupt the siblings.
-- |
-- | ```purescript
