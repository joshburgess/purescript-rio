...
-- | application can layer a `RIO.Aff.Schema` decoder on top with
...
import RIO.Aff.Core (ask, fail)
import RIO.Aff.Error (catchAll)
import RIO.Aff.Internal (RIO(..), mkRIO, rioFail)
import RIO.Aff.Schema (Schema)
...
execute stmt = do
  s <- ask (Proxy :: Proxy "sql")
  result <- liftAff (s.execute stmt)
  case result of
    Left err -> fail (Proxy :: Proxy "sqlError") err
    Right ok -> pure ok
...
queryDecode schema stmt = do
  ...
  case traverse (Schema.decode schema <<< rowToJson) rows of
    Left e -> fail (Proxy :: Proxy "sqlError") (SqlDecodeError (Schema.renderError e))
    Right xs -> pure xs
