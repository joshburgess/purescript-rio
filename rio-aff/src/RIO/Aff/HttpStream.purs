...
-- | it can sit inside `RIO.Aff.HttpClient.Request` and
-- | `RIO.Aff.HttpServer.ServerRequest`/`Response`, both of which
-- | already cross into `Aff` at the service boundary. Callers
-- | that want to build a stream from a `RIO.Aff.Stream` should drain
-- | the `RIO.Aff.Stream` into an `Aff` queue first (or run it
-- | through `RIO.Aff.Stream.intoChannel`); the boundary is one-way.
-- |
