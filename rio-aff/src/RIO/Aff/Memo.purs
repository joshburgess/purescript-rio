...
-- |
-- | This is the per-action counterpart of `RIO.Aff.Cache`, which keys on
...
-- | wrap the action in a `RIO.Aff.Schedule` retry loop before
-- | memoizing, not after.
-- |
-- | ```purescript
-- | program = do
...
import RIO.Aff.Internal (RIO(..), mkEffectRIO, mkRIO, rioFail, unRIO)
...
memoize
  :: forall r e e' a
   . RIO r e a
  -> RIO r e' (RIO r e a)
memoize action = mkEffectRIO \_ -> do
...
memoCell
  ...
  -> RIO r e a
memoCell action cell = mkRIO \r -> do
