...
-- | closure as an `Op` LIFT node so the interpreter can drive it.
module RIO.Aff.Internal
  ( RIO(..)
  , Op
  , mkRIO
  , mkEffectRIO
...
-- | and dispatches in a tight while-loop. Users never see `Op`
...
foreign import opLiftAff :: forall r e a. Aff a -> Op r e a
...
foreign import opLift
  :: forall r e a. (Record r -> Aff a) -> Op r e a
...
foreign import opSyncLift
  :: forall r e a. (Record r -> Effect a) -> Op r e a
...
foreign import _failAndStep
  :: forall r e a. OpState r e a -> Variant e -> Effect Unit
...
foreign import _isPureOp :: forall r e a. Op r e a -> Boolean
foreign import _purePayload :: forall r e a. Op r e a -> a
foreign import _isSyncOp :: forall r e a. Op r e a -> Boolean
foreign import _syncEff :: forall r e a. Op r e a -> Effect a
...
-- |
-- | Internally `RIO` wraps an `Op r e a` and is driven by the
...
-- | operation list they live as FAIL nodes and unwind cheaply via
...
mkEffectRIO f = RIO (opSyncLift f)
...
runOp env op = do
  ...
  if _isDone state then
    ...
    tailRecM step state
  where
  step :: OpState r e a -> Aff (Step (OpState r e a) (Either (Variant e) a))
  step state =
    if _isDone state then
      pure
        ( Done
...
unsafeUnRIO (RIO op) r =
  if _isPureOp op then
    pure (_purePayload op)
  else if _isSyncOp op then
    liftEffect (_syncEff op)
  else do
    result <- runOp r op
    case result of
      Right a -> pure a
...
unRIO (RIO op) r =
  if _isPureOp op then
    pure (Right (_purePayload op))
  else if _isSyncOp op then
    liftEffect (Right <$> _syncEff op)
  else
    runOp r op
...
-- | Per-operation-tag dispatch counts. Populated only when the
-- | interpreter is built with `RIO_OP_PROFILE=1` in the
...
foreign import _dumpOpCounts :: Effect OpCounts
foreign import _resetOpCounts :: Effect Unit

-- | Snapshot the per-tag dispatch counters. Reads zeros unless the
-- | profiling build of the interpreter is in effect.
dumpOpCounts :: Effect OpCounts
dumpOpCounts = _dumpOpCounts

-- | Zero the per-tag dispatch counters in place.
resetOpCounts :: Effect Unit
resetOpCounts = _resetOpCounts
...
instance applyRIO :: Apply (RIO r e) where
  apply (RIO f) (RIO x) =
    RIO (opBind f \fn -> opBind x \v -> opPure (fn v))

instance applicativeRIO :: Applicative (RIO r e) where
  pure a = RIO (opPure a)

-- | `bind` is a single BIND operation in the interpreter; the
