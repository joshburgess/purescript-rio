module Test.RIO.Aff.ClockSpec (spec) where
...
import RIO.Aff.Clock (Clock, now, sleep, timed)
import RIO.Aff.Concurrency (fork, join) as Concurrency
import RIO.Aff.Core (RIO, provideAll, runRIO')
import RIO.Aff.Test.Clock (newTestClock)

spec :: Spec Unit
spec = do
  describe "RIO.Aff.Clock + RIO.Aff.Test.Clock (Phase 7.1)" do
    describe "now" do
