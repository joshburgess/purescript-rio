module Test.RIO.Aff.TracerSpec (spec) where
...
import RIO.Aff.Core (RIO, fail, fork, join, provideAll, runRIO, runRIO')
...
spec :: Spec Unit
spec = describe "RIO.Aff.Tracer" do
  ...
  it "recording backend assigns deterministic monotonic ticks to startMs/endMs" do
    -- `RIO.Aff.Test.Tracer` module docstring promises: "Each
  ...
  it "currentSpan returns Nothing at the top-level before any withSpan opens" do
    -- `RIO.Aff.Test.Tracer` initializes the recorder's `currentRef`
