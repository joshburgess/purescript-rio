-- | Integration tests for `RIO.Fiber.Postgres.Json`. Exercises the
-- | round-trip of typed values through a real `jsonb` column.
module Test.RIO.Fiber.Postgres.JsonSpec (spec) where
...
import RIO.Fiber.Postgres.Json (JsonB(..))
...
spec conn = do
  describe "RIO.Fiber.Postgres.Json (integration)" do
