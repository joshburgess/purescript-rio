module Test.RIO.Fiber.Node.PathSpec (spec) where
...
import RIO.Fiber.Node.Path as Path

spec :: Spec Unit
spec = describe "RIO.Fiber.Node.Path" do
  ...
  it "resolve in RIO returns an absolute path" do
    absolute <- runAffThrow (Path.resolve [ "a", "b" ] "c")
