module Test.RIO.Fiber.Node.FileSystemSpec (spec) where
...
import RIO.Fiber.Layer (fromValue, provide)
import RIO.Fiber.Node.FileSystem
  ( FileSystem
  , access
  , appendTextFile
  , liveFileSystem
...
-- | filesystem. The closed `()` error row means `runAffThrow` can
...
runFs program = runAffThrow (provide (fromValue { fs: liveFileSystem }) program)
...
spec :: Spec Unit
spec = describe "RIO.Fiber.Node.FileSystem (live)" do
