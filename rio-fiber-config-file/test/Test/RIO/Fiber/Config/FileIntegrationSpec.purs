module Test.RIO.Fiber.Config.FileIntegrationSpec (spec) where
...
import RIO.Fiber.Config.File (dotenvFileSource, jsonFileSource)
...
runLoad src = do
  result <- runAffEither
    (load cfgTag src cfgDescriptor :: RIO () Err Cfg)
    {}
  case result of
...
spec :: Spec Unit
spec = describe "RIO.Fiber.Config.File integration" do
  it "loads via dotenvFileSource" do
    dir <- mkdtemp "/tmp/rio-fiber-config-file-"
