-- | File-backed `Source`s for `RIO.Fiber.Config`.
-- |
...
-- |   shape matches the way `RIO.Fiber.Config.nested` qualifies keys, so
...
-- |
-- | ```purescript
-- | import RIO.Fiber.Config (load)
-- | import RIO.Fiber.Config.File (jsonFileSource)
-- |
-- | main = launchAff_ do
...
import RIO.Fiber.Config (Source, mkSource)
