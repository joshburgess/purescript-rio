# Migrating from Effect-TS

This guide maps Effect-TS idioms to RIO. Effect-TS is a TypeScript
...
principles. This page is for Effect-TS users who want a quick
reference. The sibling `docs/migrating-from-zio.md` covers the
same ground from the Scala / ZIO angle.

## The core type

Effect-TS uses three type parameters: success, error,
requirements.
...
In Effect-TS, `R` is read as "the intersection of services I
require"; in RIO `r` is read as "the record of services I
require, indexed by name". The empty environment in Effect-TS
is `never`; in RIO it is the empty row `()`. The empty error in
Effect-TS is `never`; in RIO it is the empty row `()`.
...
Effect-TS's `Effect.sandbox` / `Effect.catchAllCause`).
...
Effect-TS tags services with `Context.Tag`:
...
present somewhere upstream. Effect-TS uses a `Context.Tag` and
...
Effect-TS's `catchTag` and RIO's `catchTag` line up almost
...
`(parse :: String)`); Effect-TS's union does the same job via
...
Effect-TS provides, implemented on top of `Effect.Aff.bracket`.
See `docs/05-resources.md` (forthcoming; today,
`src/RIO/Resource.purs` docstrings and `spikes/phase-4-review/`
are the reference) and the Phase 0.5 spike for the underlying
`Aff` guarantees.

## Concurrency

Effect-TS's `Effect.fork`, `Effect.race`, `Effect.raceAll`, and
...
Effect-TS's `Layer` and RIO's `Layer` share the same shape: an
...
direct counterpart of Effect-TS's `TestClock`: virtual time, an
...
- **The error row is structural.** `RIO r (notFound :: Int,
  ...
  tag propagates by inference. Effect-TS's preferred style is
