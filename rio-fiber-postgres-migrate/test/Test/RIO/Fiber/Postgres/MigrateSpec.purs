-- | Integration tests for `RIO.Fiber.Postgres.Migrate` against a real
-- | Postgres instance.
module Test.RIO.Fiber.Postgres.MigrateSpec (spec) where
...
import RIO.Fiber.Postgres.Migrate (migrate, sqlMigration)
...
runWith conn program =
  runAffEither
    (provideScoped (postgresLayer { connectionString: conn }) program)
    {}
...
spec conn = do
  describe "RIO.Fiber.Postgres.Migrate (integration)" do
    ...
    it "skips already-applied migrations on a second run" do
      ...
      let
        bumpAndCreate
          :: Ref Int -> String -> PG.Client -> RIO (postgres :: Postgres) DbErr Unit
