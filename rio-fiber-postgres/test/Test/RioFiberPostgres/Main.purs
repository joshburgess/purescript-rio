-- | Integration tests for `rio-fiber-postgres`.
-- |
...
-- | invoking `npx spago test -p rio-fiber-postgres`.
-- |
...
module Test.RioFiberPostgres.Main where
...
import Test.RIO.Fiber.Postgres.NotifySpec as NotifySpec
import Test.RIO.Fiber.Postgres.PoolSpec as PoolSpec
import Test.RIO.Fiber.PostgresSpec as PostgresSpec
...
main = do
  ...
  runSpecAndExitProcess [ consoleReporter ] case mConn of
    Just conn -> do
      ...
      PoolSpec.spec conn
    Nothing -> pending
      "rio-fiber-postgres integration tests: set PG_CONNECTION_STRING (e.g. via `docker compose up -d postgres`) to run"
