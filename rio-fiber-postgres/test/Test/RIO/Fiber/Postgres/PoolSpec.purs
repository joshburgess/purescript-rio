-- | Integration tests for `RIO.Fiber.Postgres.Pool` against a real
-- | Postgres instance.
module Test.RIO.Fiber.Postgres.PoolSpec (spec) where
...
import RIO.Fiber.Postgres (PgError, Postgres, pgErrorMessage)
import RIO.Fiber.Postgres.Layer (postgresLayer)
import RIO.Fiber.Postgres.Pool (PoolStats, poolStats, warmup)
...
spec conn = do
  describe "RIO.Fiber.Postgres.Pool (integration)" do
