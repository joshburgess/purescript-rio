...
-- | `node-postgres` / `pg`) expressed as `rio-fiber` combinators.
-- |
...
-- | in `RIO.Fiber.Postgres.Layer`.
module RIO.Fiber.Postgres
  ( Postgres(..)
  , PgError(..)
  , pgErrorMessage
  , withClient
...
-- |
-- | Build one with `RIO.Fiber.Postgres.Layer.postgresLayer`.
...
-- | tag carrying `PgError`. Aff defects propagate as fiber defects.
fromExcept
  :: forall sym r e e' a
   . IsSymbol sym
  ...
  -> PG.Except Aff a
  -> RIO r e a
fromExcept sym action = do
  res <- fromAff (runExceptT action)
  case res of
    Right a -> pure a
    Left es -> fail (Variant.inj sym (PgError es) :: Variant e)
...
withClientUsing
  ...
  -> (PG.Client -> RIO r e a)
  -> RIO r e a
withClientUsing sym pool use = scoped \scope -> do
  client <- acquireRelease scope
