...
-- |
-- | The breaker counts typed errors only. Defects (`die`d throws
-- | and other `Aff` exceptions) propagate through unchanged and
-- | do not affect state; their failure model is "stop the world",
-- | which the breaker's fail-fast model does not improve on. If
-- | you want a defect to count as a failure, sandbox it at the
-- | call site and rethrow it as a typed error.
-- |
-- | ```purescript
-- | -- protect an outbound call against cascading failures
-- | breaker <- CircuitBreaker.make { maxFailures: 5, resetTimeout: Milliseconds 30_000.0 }
...
-- | ```
-- |
-- | Like `RateLimiter`, the breaker reads time through `RIO.Clock`
-- | so tests can drive it deterministically with `RIO.Test.Clock`.
module RIO.CircuitBreaker
  ( CircuitBreaker
  , Config
  , Phase(..)
  , Snapshot
...
import RIO.Clock (Clock, now)
import RIO.Core (RIO, catchAll, fail, rethrow)
...
-- | Compute the phase the breaker should be in given the current
-- | time. `Open` ages into `HalfOpen` after `resetTimeoutMs`; the
-- | other phases stay put.
...
-- | Decide whether the call should proceed given the current
-- | (already-expired) phase. Returns `true` to proceed and `false`
-- | to fail fast.
...
-- | After a successful call, transition to `Closed` and clear the
-- | failure counter.
...
-- | After a typed-error call, bump the failure counter, then trip
-- | to `Open` if the threshold has been reached or we were
-- | already half-open.
...
-- | Internal helper: compute whether to admit, update the phase
-- | to reflect time, and return the (boolean, updated) pair.
admit
  :: Number
  -> State
