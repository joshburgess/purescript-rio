...
-- |
-- | `RIO.OpenApi` is the data-only side of the API: a small set of
...
-- |
-- | The module reuses `RIO.Schema`'s `toJsonSchema` for body and
-- | parameter shapes, so a single `Schema a` value drives both
-- | wire decoding (`RIO.Schema.decode`) and the OpenAPI fragment.
-- |
...
-- tags that `RIO.HttpClient.Method` carries.
