...
-- | counterpart to `RIO.Deferred`.
-- |
-- | `RIO.Deferred` is built on `AVar`: it composes with `RIO` and
...
import RIO.STM (STM, TRef, newTRef, readTRef, retry, throwSTM, writeTRef)
