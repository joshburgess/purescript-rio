...
-- | service.
-- |
-- | Where the temporal combinators in `RIO.Stream` are content with
-- | wall-clock waits (none today), the combinators here all read
-- | virtual time from `RIO.Clock` so they are deterministic against
-- | `RIO.Test.Clock`.
-- |
...
-- | timer against an upstream pull via `RIO.Concurrency.race`. When
