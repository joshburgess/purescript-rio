...
-- |
-- | `RIO.HttpClient` defines the *shape* of an HTTP transport so
...
-- | `RIO.Schedule.retry` for retry policies, with `RIO.Tracer`
-- | for span instrumentation, or with `RIO.Logger` for structured
