...
-- | interop with `RIO.Stream` and `RIO.Sink`, and `pipe` /
...
import RIO.Sink (Sink, Step(..), unSink) as Sink
import RIO.Stream (Stream, Step(..), unStream) as Stream
...
-- | Lift a `RIO.Stream.Stream r e a` into a channel: a source
...
-- | Lift a `RIO.Sink.Sink r e i a` into a channel: a sink that
