...
-- |
-- | `RIO.Ref` deliberately omits a `modifyM`-style combinator
-- | because any `RIO`-typed update body can yield to other fibers
-- | between read and write, which would race. `Ref.Synchronized`
-- | is the principled answer: it pairs a plain `Ref` with a
-- | `Semaphore` of one permit and serialises every effectful
-- | update behind that permit. Reads and pure updates still take
-- | the permit, so a long-running effectful update will block
-- | concurrent readers; this is the same trade-off ZIO's
-- | `Ref.Synchronized` and Effect's `SynchronizedRef` make.
-- |
...
-- | other fibers. For pure updates, prefer `RIO.Ref` directly;
...
import RIO.Internal (RIO(..), mkEffectRIO)
...
new value = mkEffectRIO \_ -> do
  ref <- ERef.new value
  sem <- Semaphore.make 1
  pure (SynchronizedRef { ref, sem })

-- | `Effect`-typed variant for callers that allocate at the top
-- | of `main`.
...
write (SynchronizedRef s) value = Semaphore.withPermit s.sem do
  mkEffectRIO \_ -> ERef.write value s.ref
...
modify (SynchronizedRef s) f = Semaphore.withPermit s.sem do
  mkEffectRIO \_ -> ERef.modify f s.ref
...
modifyM (SynchronizedRef s) f = Semaphore.withPermit s.sem do
  current <- mkEffectRIO \_ -> ERef.read s.ref
  next <- f current
  mkEffectRIO \_ -> do
