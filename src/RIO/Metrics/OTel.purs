-- | Shape `RIO.Metrics` observations into the OTLP/JSON envelope
-- | used by OpenTelemetry collectors.
-- |
-- | The companion piece of `RIO.Tracer.OTel`: the exporter is pure,
-- | takes an in-process list of recorded observations (the kind
-- | `RIO.Test.Metrics.newRecordingMetrics`'s `snapshot` action
...
-- | `RIO.HttpClient.post` and a `withJsonBody`.
-- |
...
import RIO.Test.Metrics (MetricKind(..), MetricRecord)
