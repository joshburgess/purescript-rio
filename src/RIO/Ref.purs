...
-- | (`RIO.Ref.new`, `RIO.Ref.modify`, ...) and the consistent row
-- | of `RIO r e`. If you only need state inside a single
-- | transaction, prefer `RIO.STM.TRef`. If you need scoped
-- | overrides on the dynamic extent of a block, prefer
-- | `RIO.Local`. Use `Ref` when you want plain shared state
...
-- | latest value without races, use `RIO.STM.TRef` inside an
